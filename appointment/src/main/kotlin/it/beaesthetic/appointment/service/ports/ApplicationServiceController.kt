...
class ApplicationServiceController(
    ...
    override fun createService(
        ...
        val service = AppointmentService(
            ...
            color = createServiceRequestDto.color?.let {
                Color(it)
            }
        )
    ...
    override fun getAllServices(): Uni<List<ServiceDto>> = uniWithScope {
        applicationServiceRepository.findAll()
            .map {
                ServiceDto(
                    id = it.id,
                    name = it.name,
    ...
    override fun searchService(
        ...
        applicationServiceRepository.searchByName(text ?: "", limit)
            .map {
                ServiceDto(
                    id = it.id,
                    name = it.name,
    ...
    override fun updateService(serviceId: String, updateServiceRequestDto: UpdateServiceRequestDto): Uni<ServiceDto> =
        uniWithScope {
            val appointmentService = applicationServiceRepository.findById(serviceId)
                ?.let {
                    it.copy(
            ...
            applicationServiceRepository.save(appointmentService).map {
                ServiceDto(
                    id = it.id,
                    name = it.name,
                    price = it.price.toBigDecimal(),
