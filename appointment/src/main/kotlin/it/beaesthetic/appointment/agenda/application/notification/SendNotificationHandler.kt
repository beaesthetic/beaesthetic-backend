...
class SendNotificationHandler(
    ...
    suspend fun handle(command: SendNotificationCommand) {
        ...
        if (customer?.phoneNumber == null) {
            log.info("Attendee ${customer?.customerId} has no valid contacts, not sending notification")
        } else {
            notificationService.trackAndSendNotification(notification, customer.phoneNumber)
                .onSuccess {
                    log.info("Sent notification for event ${notification.event.id}")
                }
                .onFailure {
