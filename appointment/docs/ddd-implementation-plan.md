...
## Macro step 6: operational cutover

Status: pending

Goal: deploy directly on the new model with an explicit manual migration.

Tasks:

- Deploy schema migration.
- Run `appointment backfill-agenda-model` dry-run.
- Run `appointment backfill-agenda-model --execute`.
- Run `appointment schedule-future-reminders` to recreate eligible River jobs.
- Deploy runtime that reads/writes the new model.
- Run smoke checks for create/reschedule/cancel/reminder/outcome.

## Macro step 7: cleanup legacy

Status: pending

Goal: remove legacy storage after stabilization.

Tasks:

- Drop `pending_notifications`.
- Drop appointment-specific columns from `agenda_events`.
- Remove legacy sqlc queries.
- Remove legacy `domain.AgendaEvent` adapter.
