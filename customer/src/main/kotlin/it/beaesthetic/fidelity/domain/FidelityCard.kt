...
@JvmInline value class CustomerId(val id: String)
...
class FidelityCard(
    ...
    companion object {
        fun ofCustomer(customerId: CustomerId) =
            FidelityCard(UUID.randomUUID().toString(), customerId)
    }
    ...
    fun addPurchase(purchase: Purchase): Voucher? =
        when (purchase) {
    ...
    fun useVoucher(voucherId: VoucherId): Result<FidelityCard> {
        ...
        return when {
            ...
            foundVoucher.isUsed ->
                Result.failure(IllegalStateException("Voucher with id $voucherId is already used"))
            else ->
                Result.success(
                    FidelityCard(
                        id = id,
