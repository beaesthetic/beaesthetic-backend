...
class FidelityCardRepositoryImpl(
    ...
    override suspend fun findAll(): List<FidelityCard> {
        return panacheFidelityCardRepository.findAll().list().awaitSuspending().map { toDomain(it) }
    }
    ...
    private object EntityMapper {
        fun toDomain(entity: FidelityCardEntity) =
            FidelityCard(
                id = entity.id,
