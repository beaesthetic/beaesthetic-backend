...
@ApplicationScoped
class PanacheCustomerRepository : ReactivePanacheMongoRepository<CustomerEntity>
...
class CustomerRepositoryImpl(
    ...
    override suspend fun findById(id: CustomerId): Customer? {
        return panacheCustomerRepository.find("_id", id.value)
            .firstResult()
    ...
    override suspend fun save(customer: Customer): Customer {
        val searchTerms = listOf(customer.name, customer.surname).filter { it.trim().isNotEmpty() }.joinToString(" ")
        return panacheCustomerRepository.persistOrUpdate(
            CustomerEntity(
                ...
                searchGrams = SearchGram.Default.nGrams(
                    text = searchTerms,
                    minLength = 3
                ).joinToString(" ")
            )
        ).awaitSuspending().let { customer }
    }
    ...
    override suspend fun findByKeyword(keyword: String, maxResults: Int): List<Customer> {
        ...
        val project: Bson = Document("score", Document("\$meta", "textScore"))
            .append("searchGrams", 0L)
        val sort: Bson = Document("score", Document("\$meta", "textScore"))

        return panacheCustomerRepository.mongoCollection().find(
            FindOptions().filter(filter).projection(project).sort(sort).limit(maxResults)
        ).map { toDomain(it) }.collect().asList().awaitSuspending()
    }
