...
@ApplicationScoped
class NotificationQueueConsumer(private val notificationService: NotificationService) {
    ...
    suspend fun handle(message: Message<JsonObject>): CompletionStage<Void>? {
        ...
        val event =
            kotlin
                .runCatching { message.payload.mapTo(NotificationEvent::class.java) }
    ...
    suspend fun handleNotificationCreated(
        event: Message<NotificationCreated>
    ): CompletionStage<Void>? {
        return notificationService
            .sendNotification(event.payload.notificationId)
            .onSuccess {
