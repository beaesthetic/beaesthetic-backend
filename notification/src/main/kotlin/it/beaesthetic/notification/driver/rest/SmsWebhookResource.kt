    ...
    override suspend fun smsGatewayNotify(eventNotificationDtoDto: EventNotificationDtoDto) {
        when (eventNotificationDtoDto.eventType) {
            EventNotificationTypeDto.SUCCEEDED -> {
                log.info("Received SMS delivery success event for ${eventNotificationDtoDto.data?.id}")
                val notificationId = eventNotificationDtoDto.metadata
                    ?.get(SmsNotificationProvider.NOTIFICATION_ID_METADATA)
                if (notificationId == null) {
                    log.error("Missing mandatory metadata [${SmsNotificationProvider.NOTIFICATION_ID_METADATA}]")
            ...
            EventNotificationTypeDto.FAILED -> {
                log.info("Received SMS delivery failure event for ${eventNotificationDtoDto.data?.id}. Doing nothing")
            }
            else -> {
