...
class SmsNotificationProvider(
    ...
    override suspend fun send(notification: Notification): Result<ChannelMetadata> = runCatching {
        val channel = notification.channel as Sms
        val response = smsApi.sendSms(
            notification.id,
            SendSmsRequest()
                .to(channel.phone)
                .from(fromNumber)
        ...
        log.info("Successfully send sms notification ${it.providerResourceId}")
    }.onFailure {
        log.error("Error sending sms notification ${it.message}", it)
    }
}