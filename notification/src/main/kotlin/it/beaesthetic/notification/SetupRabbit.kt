...
class SetupRabbit(
    ...
    @Startup
    @Priority(1)
    fun setupRetryQueue() = runBlocking {
        val vertx = Vertx.vertx()
        try {
            val rabbitClient = RabbitMQClient.create(
                vertx,
            ...
            rabbitClient.start()
                ...
                .flatMap {
                    rabbitClient.queueDeclare(
                        queueName, true, false, false, JsonObject.of(
                            "x-dead-letter-exchange", retryQueue,
                        )
                    )
                }
                .flatMap {
                    rabbitClient.queueBind(queueName, "amq.direct", queueName)
                }
                .flatMap {
                    rabbitClient.queueBind(retryQueue, retryExchange, retryQueue)
                }
                .onItem().invoke { _ ->
                    log.info("Setup rabbit done for $queueName")
                }
                .onFailure().invoke { e ->
                    log.error("Failed to setup queue $queueName", e)
                }
                .awaitSuspending()
        } finally {
